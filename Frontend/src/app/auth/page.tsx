...
export default function AuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || "Đã có lỗi xảy ra. Thử lại nhé!");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center px-8 text-center w-full max-w-md">
          <p className="text-lg font-medium opacity-90 text-[#437d6e]">Welcome to ShareMoney</p>
          <h1 className="text-[32px] sm:text-[38px] font-extrabold leading-tight mt-1 mb-2 text-[#437d6e]">
            Làm chủ tài chính,<br />Chia tiền dễ dàng
          </h1>
  ...
  return (
    ...
    >
      ...
      <main className="flex-1 flex flex-col items-center justify-center w-full max-w-md px-6 -mt-8">
        ...
        <h1 className="text-3xl font-extrabold text-[#437d6e] mb-1">ShareMoney</h1>
        <p className="text-sm text-[#437d6e]/70 mb-8">Quản lý chi tiêu thông minh & Chia tiền nhóm dễ dàng.</p>
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-3">
          {tab === "register" && (
            <div>
              <label className="text-xs font-semibold text-[#437d6e] block mb-1">Tên của bạn</label>
              <input
                type="text"
          ...
          <div>
            <label className="text-xs font-semibold text-[#437d6e] block mb-1">Email</label>
            <input
              type="email"
          ...
          <div>
            <label className="text-xs font-semibold text-[#437d6e] block mb-1">Mật khẩu</label>
            <input
              type="password"
              placeholder="••••••••"
              value={form.password}
          ...
          <button
            ...
            style={{ backgroundColor: "#6ebda9" }}
          >
            {loading ? "Đang xử lý..." : tab === "login" ? "Vào trong nào →" : "Tiếp tục →"}
          </button>
        </form>
      </main>
