...
import { AddExpenseDrawer } from "@/components/ngan-keo-them-chi-tieu";
import { EditExpenseDrawer } from "@/components/ngan-keo-sua-chi-tieu";
import { AddMemberDialog } from "@/components/hop-thoai-them-thanh-vien";
import { SettleDebtDialog } from "@/components/hop-thoai-thanh-toan-no";
import { RemindDebtDialog } from "@/components/hop-thoai-nhac-no";
import { ExpenseChart } from "@/components/bieu-do-chi-tieu";
import { ConfirmCashDialog } from "@/components/hop-thoai-xac-nhan-tien-mat";
...
export default function GroupDetailPage({
  ...
  const [activeTab, setActiveTab] = useState<
    "expenses" | "balances" | "members" | "history"
  >("expenses");
  const [historyFilter, setHistoryFilter] = useState<"group" | "personal">(
    "group",
  );
  ...
  const fetchGroupData = async (reset = false) => {
    try {
      ...
      const [groupRes, expensesRes, debtsRes, pendingRes, pendingSentRes] =
        await Promise.all([
          api.get(`/groups/${id}`),
  ...
  const myDebts =
    debts?.transactions?.filter((t: any) => t.from.id === currentUser?.id) ??
    [];
  const owedToMe =
    debts?.transactions?.filter((t: any) => t.to.id === currentUser?.id) ?? [];
  const otherDebts =
    debts?.transactions?.filter(
      (t: any) => t.from.id !== currentUser?.id && t.to.id !== currentUser?.id,
    ) ?? [];
  ...
  if (!group) {
    return (
      ...
      >
        ...
        <button
          onClick={() => router.push("/")}
          className="text-[#45b39d] font-semibold mt-4"
        >
          ← Về trang chủ
        </button>
      </div>
  ...
  return (
    ...
    >
      {/* ─── HERO HEADER ─── */}
      <header
        className="relative pt-3 pb-6 px-4"
        style={{
          background:
            "linear-gradient(180deg, #B3E5D1 0%, #B3E5D1 70%, #FDF9F0 100%)",
        }}
      >
        {/* Back + Invite */}
        ...
        <div className="flex -space-x-2 mt-3 ml-1">
          {group.members.slice(0, 5).map((m) => (
            <div
              key={m.id}
              className="w-8 h-8 rounded-full border-2 border-white overflow-hidden bg-[#B3E5D1]"
            >
              {m.user.avatarUrl ? (
                <img
      ...
      <nav className="px-4 py-3 flex gap-2 overflow-x-auto no-scrollbar sticky top-0 z-10 bg-[#FDF9F0]">
        {(["expenses", "balances", "history", "members"] as const).map(
          (tab) => (
            <button
              key={tab}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <AddExpenseDrawer
                  ...
                  onExpenseCreated={() => fetchGroupData(true)}
                />
              </div>
            </div>
            ...
            0 ? (
              <div className="flex flex-col items-center py-12 text-center">
                <div className="w-16 h-16 rounded-full bg-[#B3E5D1] flex items-center justify-center text-3xl mb-3">
                  🧾
                </div>
                <p className="font-bold text-gray-700">Chưa có hóa đơn nào</p>
                <p className="text-sm text-gray-400 mt-1">
                  Thêm hóa đơn đầu tiên ngay!
                </p>
              </div>
            ) : (
              expenses
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-all">
                      <div className="p-4 flex items-center gap-4">
                        ...
                        <div
                          ...
                          style={{
                            backgroundColor:
                              CATEGORY_BG[expense.category] ?? "#f3f4f6",
                          }}
                        >
                      ...
                      <div className="border-t border-gray-100 px-4 py-2.5 bg-gray-50/50 flex justify-between items-center">
                        <p className="text-sm text-gray-600">
                          ...
                          đã trả · chia {expense.splitCount} người
                        </p>
              ...
              {expenses
                .filter((e) => {
                  ...
                  if (e.category === "SETTLEMENT") {
                    return (
                      e.payer.id === currentUser?.id ||
                      e.title.includes(currentUser?.name ?? "")
                    );
                  }
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">
                        {expense.category === "SETTLEMENT" ? (
                          ...
                          <>
                            ...
                            đã chi {formatCurrency(expense.amount)}
                          </>
                        )}
                      </p>
                    </div>
        ...
        {activeTab === "balances" && (
          <div className="space-y-4 mt-2">
            ...
            <div className="flex gap-3">
              ...
              >
                <div
                  className="text-center py-2.5 font-bold text-sm text-white"
                        ...
                        >
                          ...
                          {!isPending ? (
                            ...
                            >
                              ...
                              >
                                <button
                                  className="w-full text-xs font-bold text-white py-1.5 px-4 rounded-xl transition-all active:scale-95 shadow-sm"
                                  style={{ backgroundColor: "#7ED3B2" }}
                                >
                                  Nhắc nợ
                                </button>
                ...
                <div className="p-3 space-y-3">
                  {myDebts.length === 0 ? (
                    ...
                    myDebts.map((t: any, i: number) => {
                      const isPendingSent = pendingSentCreditors.includes(
                        t.to.id,
                      );
                      return (
                        <div
                          key={i}
                          className={`bg-white rounded-2xl p-3 flex flex-col items-center text-center shadow-sm ${isPendingSent ? "border-2 border-amber-300 bg-amber-50" : "border border-transparent"}`}
                        >
                          <div
                            className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 font-bold ${isPendingSent ? "bg-amber-100 text-amber-600" : "bg-[#FFEDE1] text-[#FF9E7D]"}`}
                          >
                            {t.to.name.charAt(0)}
                          </div>
                          <p className="text-xs font-semibold text-gray-700">
                          ...
                          <p
                            className={`text-lg font-black my-1 ${isPendingSent ? "text-amber-500" : "text-[#FF9E7D]"}`}
                          >
                            {new Intl.NumberFormat("vi-VN").format(t.amount)}đ
                          </p>
                          {isPendingSent ? (
                            <button
                              disabled
                              className="w-full mt-2 text-xs font-bold py-1.5 px-4 rounded-xl shadow-sm flex items-center justify-center gap-1 bg-amber-200 text-amber-700 cursor-not-allowed"
                            >
                              <svg
            ...
            {otherDebts.length > 0 && (
              ...
              >
                <div
                  className="text-center py-2.5 font-bold text-sm text-slate-500"
                ...
                <div className="p-3 space-y-3">
                  {otherDebts.map((t: any, i: number) => (
                    <div
                      key={i}
                      className="bg-white rounded-2xl p-3 flex flex-row items-center justify-between shadow-sm border border-slate-100"
                    >
              ...
              >
                ...
                {member.role === "ADMIN" && (
                  <span
                    ...
                    style={{ backgroundColor: "#B3E5D1", color: "#437d6e" }}
                  >
                    Admin
                  </span>
                )}
      ...
      <ConfirmCashDialog
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            toast.success(
              `Đã xác nhận nhận tiền từ ${confirmCash.debtorName}! 🎉`,
            );
            setConfirmCash(null);
            fetchGroupData(true);
          } catch (err: any) {
