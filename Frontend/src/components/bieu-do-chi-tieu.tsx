...
function getColor(category: string, index: number): string {
  return (
    CATEGORY_COLORS[category] ?? DEFAULT_COLORS[index % DEFAULT_COLORS.length]
  );
}
...
export function ExpenseChart({ expenses }: ExpenseChartProps) {
  ...
  const CX = 110,
    CY = 110,
    R = 98,
    IR = 75; // Increased size and relative thickness
  let cursor = 0;
          ...
          <div className="relative shrink-0">
            <svg width={220} height={220} viewBox="0 0 220 220">
              ...
              <circle
                ...
                r={IR}
                fill="white"
                className="dark:fill-slate-900 pointer-events-none"
              />
          ...
          <div className="flex-1 space-y-2 w-full">
            {slices.map((sl, i) => (
              <div
                ...
                style={{
                  background:
                    hoveredIdx === i ? `${sl.color}18` : "transparent",
                }}
                onMouseEnter={() => setHoveredIdx(i)}
                onMouseLeave={() => setHoveredIdx(null)}
              >
