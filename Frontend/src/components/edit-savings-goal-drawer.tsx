...
export function EditSavingsGoalDrawer({ open, onOpenChange, goal, onSaved }: EditSavingsGoalDrawerProps) {
      ...
      <DrawerContent className="bg-gradient-to-b from-[#dff5ef] to-[#f4fdfb] rounded-t-[40px] px-2 pb-6 border-none">
        <div className="mx-auto w-full max-w-sm pt-2">
          <DrawerHeader className="text-center pb-6">
            <DrawerTitle className="text-2xl font-bold text-slate-800">Sửa mục tiêu</DrawerTitle>
          </DrawerHeader>
          
          <div className="px-4">
            <form id="edit-savings-goal-form" onSubmit={handleSubmit} className="space-y-6">
              
              <div className="space-y-2">
                <Label htmlFor="edit-goal-name" className="text-[13px] font-medium text-slate-700 px-1">Tên mục tiêu</Label>
                <Input
                  id="edit-goal-name"
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-goal-amount" className="text-[13px] font-medium text-slate-700 px-1">Số tiền mục tiêu (VNĐ)</Label>
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-due-date" className="text-[13px] font-medium text-slate-700 px-1">Ngày đến hạn (Không bắt buộc)</Label>
                <Input
                  id="edit-due-date"
                  type="date"
                  value={deadlineDate}
        ...
        {showConfirmDelete && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
            <div className="bg-white rounded-3xl p-6 w-full max-w-[320px] shadow-2xl animate-in zoom-in-95 duration-200">
              <h3 className="text-[19px] font-extrabold text-slate-800 mb-2 text-center">Xóa mục tiêu này?</h3>
              <p className="text-[13px] text-slate-500 text-center mb-6">Toàn bộ số tiền sẽ được hoàn lại vào ví của bạn. Bạn có chắc chắn muốn xóa không?</p>
