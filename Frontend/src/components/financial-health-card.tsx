    ...
    <div className="bg-white rounded-[28px] p-5 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-gray-100 relative overflow-hidden group">
      ...
      <div className="relative z-10 flex flex-col gap-5">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-[17px] font-extrabold text-gray-800 tracking-tight">Sức khỏe Tài chính</h3>
            <p className="text-[11px] text-gray-400 font-medium">Đánh giá dựa trên thu chi và nợ</p>
          </div>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-12 gap-5 items-center">
          ...
          <div className="xs:col-span-4 flex justify-center relative">
            <div className="relative w-24 h-24 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className={`text-[26px] leading-none font-black ${colorClass}`}>{score}</span>
                <span className="text-[8px] text-gray-400 font-black tracking-wider mt-0.5">ĐIỂM</span>
              </div>
            </div>
          </div>
          ...
          <div className="xs:col-span-8 space-y-3">
            {[
              { label: "Tỷ lệ tích lũy", score: health.savingsRatioScore, icon: "🌱", color: health.savingsRatioScore >= 20 ? "bg-emerald-500" : health.savingsRatioScore >= 10 ? "bg-amber-400" : "bg-rose-500" },
              { label: "Tuân thủ ngân sách", score: health.budgetAdherenceScore, icon: "📊", color: health.budgetAdherenceScore >= 20 ? "bg-emerald-500" : health.budgetAdherenceScore >= 10 ? "bg-amber-400" : "bg-rose-500" },
              { label: "Kiểm soát nợ", score: health.debtToIncomeScore, icon: "💸", color: health.debtToIncomeScore >= 20 ? "bg-emerald-500" : health.debtToIncomeScore >= 10 ? "bg-amber-400" : "bg-rose-500" },
              { label: "Quỹ dự phòng", score: health.emergencyFundScore, icon: "🛡️", color: health.emergencyFundScore >= 20 ? "bg-emerald-500" : health.emergencyFundScore >= 10 ? "bg-amber-400" : "bg-rose-500" }
            ].map((item, idx) => {
                  ...
                  <div className="flex justify-between items-center text-[11px] font-bold text-gray-600">
                    ...
                    <span className="flex items-center gap-1.5">
                      <span className={`text-[10px] font-black uppercase tracking-wider ${
                        item.score >= 20 ? "text-emerald-500" :
                        item.score >= 15 ? "text-blue-500" :
                        item.score >= 10 ? "text-amber-500" : "text-rose-500"
                      }`}>
                        {item.score >= 20 ? "Tuyệt vời" : item.score >= 15 ? "Khá tốt" : item.score >= 10 ? "Tạm ổn" : "Báo động"}
                      </span>
                      <div className="flex gap-0.5 ml-1">
                        {[...Array(10)].map((_, i) => {
                          const score10 = Math.round((item.score / 25) * 10);
                          const isActive = i < score10;
                          const activeColor = item.score >= 20 ? "bg-emerald-500" : item.score >= 15 ? "bg-blue-500" : item.score >= 10 ? "bg-amber-500" : "bg-rose-500";
                          return (
                            <div key={i} className={`w-1.5 h-3.5 rounded-[2px] transition-all duration-500 ${isActive ? activeColor : "bg-slate-200"}`} />
                          );
                        })}
                      </div>
                    </span>
