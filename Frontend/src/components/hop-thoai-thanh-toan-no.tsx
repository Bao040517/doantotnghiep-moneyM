...
export function SettleDebtDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success(
        `Đã gửi thông báo cho ${toUser.name}. Vui lòng chờ xác nhận!`,
      );
      setOpen(false);
      onSettle();
    } catch (error: any) {
  ...
  const isManualInput =
    !toUser.bankBin ||
    !toUser.bankAccountNo ||
    tempBankBin !== toUser.bankBin ||
    tempBankAccount !== toUser.bankAccountNo;
            ...
            <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-slate-900 rounded-lg my-2 border border-slate-100 dark:border-slate-800">
              <Avatar className="h-10 w-10 border border-slate-200">
                <AvatarImage
                  src={
                    toUser.avatarUrl ||
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(toUser.name)}`
                  }
                />
              ...
              <TabsContent value="transfer" className="mt-4 space-y-4">
                    ...
                    {hasDynamicQr ? (
                      <div className="text-center space-y-2">
                        ...
                        <p className="text-xs font-semibold text-emerald-600">
                          Quét mã VietQR (Tự động cập nhật)
                        </p>
                      </div>
                    ) : (
                      <div className="text-center space-y-2">
                ...
                <div className="w-full bg-slate-50 rounded-2xl p-4 text-[12px] space-y-3 border border-slate-100 dark:bg-slate-900 dark:border-slate-800">
                  <div className="flex flex-col gap-1.5">
                    <span
                      className={`font-medium text-[11px] ${errors.bankBin ? "text-rose-500" : "text-slate-500"}`}
                    >
                      Ngân hàng nhận
                    </span>
                    <div
                      className={`bg-white dark:bg-slate-950 rounded-lg border relative ${errors.bankBin ? "border-rose-500 ring-1 ring-rose-500/20" : "border-slate-200 dark:border-slate-700"}`}
                    >
                      <select
                        value={tempBankBin}
                        onChange={(e) => {
                          setTempBankBin(e.target.value);
                          setErrors((prev: any) => ({
                            ...prev,
                            bankBin: false,
                          }));
                  ...
                  <div className="flex flex-col gap-1.5">
                    <span
                      className={`font-medium text-[11px] ${errors.bankAccount ? "text-rose-500" : "text-slate-500"}`}
                    >
                      Số tài khoản nhận
                    </span>
                    <input
                      ...
                      onChange={(e) => {
                        setTempBankAccount(e.target.value);
                        setErrors((prev: any) => ({
                          ...prev,
                          bankAccount: false,
                        }));
                  ...
                  <div className="flex flex-col gap-1.5">
                    <span
                      className={`font-medium text-[11px] ${errors.accountName ? "text-rose-500" : "text-slate-500"}`}
                    >
                      Tên người nhận
                    </span>
                    <input
                  ...
                  <div className="border-t border-slate-200/60 dark:border-slate-800 mt-2 pt-3 flex flex-col gap-1.5">
                    <span
                      className={`font-bold text-[12px] ${errors.amount ? "text-rose-500" : "text-slate-500"}`}
                    >
                      Số tiền thực tế:
                    </span>
                    <div className="relative">
                      ...
                      />
                      <span className="absolute right-3 top-1/2 -translate-y-1/2 text-rose-600 font-black">
                        đ
                      </span>
                    </div>
                  </div>
              ...
              <TabsContent value="cash" className="mt-4">
                <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50/50 dark:bg-slate-900/50 text-center">
                  ...
                  <p className="text-sm text-slate-500 mt-2 max-w-[250px]">
                    Hãy đưa trực tiếp số tiền{" "}
                    <span className="font-bold text-slate-900 dark:text-white">
                      {formatCurrency(amount)}
                    </span>{" "}
                    cho {toUser.name}. Sau khi đưa tiền, hãy nhấn Xác nhận bên
                    dưới.
                  </p>
                </div>
              </TabsContent>
          ...
          <DrawerFooter>
            ...
            >
              {isSettling ? (
                ...
                <>
                  <CheckCircle2 className="w-4 h-4 mr-2" /> Báo đã chuyển tiền
                </>
              )}
            </Button>
            <DrawerClose asChild>
