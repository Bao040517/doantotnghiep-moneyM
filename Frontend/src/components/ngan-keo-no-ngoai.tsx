...
import { AddLoanDrawer } from "@/components/ngan-keo-them-no";
import { EditLoanDrawer } from "@/components/ngan-keo-sua-no";
...
export function ExternalLoansDrawer({
  open,
  onOpenChange,
  onSaved,
}: ExternalLoansDrawerProps) {
  const [loans, setLoans] = useState<ExternalLoanDTO[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedLoan, setSelectedLoan] = useState<ExternalLoanDTO | null>(
    null,
  );
        ...
        <DrawerContent className="bg-[#f4f6f9] h-[90vh]">
          <div className="mx-auto w-full max-w-md h-full flex flex-col">
            ...
            <DrawerHeader className="bg-white border-b border-gray-100 shrink-0 text-center pb-5 pt-4 rounded-t-2xl shadow-sm z-10 relative">
              ...
              <DrawerTitle className="text-xl font-extrabold text-gray-800">
                Sổ Vay Nợ & Tiết Kiệm
              </DrawerTitle>
              ...
              {isLoading && loans.length === 0 ? (
                ...
                <div className="text-center py-12 bg-white rounded-[24px] border border-dashed border-slate-200">
                  ...
                  <p className="text-[10px] text-gray-400 mt-0.5">
                    Nhấp nút bên trên để tạo khoản ghi chép mới
                  </p>
                </div>
              ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <h4 className="text-sm font-bold text-gray-800 truncate pr-8">
                          {loan.counterpartyName}
                        </h4>
                        <div className="flex items-center gap-1.5 mt-0.5 text-[10px] text-gray-400 font-medium">
                          <span>
                            {loan.type === "LENT" ? "Cho vay" : "Đi vay"}
                          </span>
                          {loan.interestRate > 0 && (
                            <span className="bg-amber-50 text-amber-600 px-1 py-0.25 rounded text-[9px] font-bold">
                              Lãi: {loan.interestRate}%/năm
                            </span>
                          )}
                          {loan.isSettled && (
                            <span className="bg-slate-100 text-slate-600 px-1 py-0.25 rounded text-[9px] font-bold">
                              Đã trả xong
                            </span>
                          )}
                        </div>
