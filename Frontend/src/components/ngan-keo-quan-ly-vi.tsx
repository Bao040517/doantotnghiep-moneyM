...
export function WalletManagerDrawer({
  ...
  const handleDelete = async (id: string, wName: string) => {
    if (
      !confirm(
        `Bạn có chắc muốn xóa ví "${wName}"? Các giao dịch liên quan có thể bị ảnh hưởng.`,
      )
    )
      return;
    try {
            ...
            <>
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-[13px] text-slate-500 font-medium">
                    Quản lý các tài khoản & ví của bạn
                  </p>
                </div>
                <button
              ...
              {loading ? (
                <div className="flex justify-center py-10">
                  <div className="w-6 h-6 rounded-full border-4 border-slate-200 border-t-[#2BA76F] animate-spin" />
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {wallets.map((w) => {
                    const linkedBank = w.bankBin
                      ? BANKS.find((b) => b.bin === w.bankBin)
                      : null;
                    return (
                      <div
                        key={w.id}
                        className="bg-white rounded-2xl p-4 shadow-[0_4px_15px_-5px_rgba(0,0,0,0.05)] border border-slate-100 flex items-center gap-4 group"
                      >
                        {/* Logo */}
                        {linkedBank ? (
                          <div
                            className={`w-12 h-12 rounded-[14px] flex flex-col items-center justify-center shadow-sm shrink-0 ${linkedBank.color} ${linkedBank.text}`}
                          >
                            <span className="text-[14px] font-black tracking-tight">
                              {linkedBank.shortName}
                            </span>
                          </div>
                        ) : (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[14px] font-black text-[#2BA76F] mt-1">
                        ...
                        <div className="flex flex-col gap-2 shrink-0">
                          <button
                            ...
                            className="w-8 h-8 rounded-full bg-slate-50 text-slate-400 hover:text-[#2BA76F] hover:bg-emerald-50 flex items-center justify-center transition-colors"
                          >
                            <svg
                              className="w-4 h-4"
                              fill="none"
                ...
                {!editingWallet && (
                  <div className="bg-white p-4 rounded-[20px] shadow-sm border border-slate-100">
                    ...
                    <div className="flex items-center">
                      ...
                      />
                      <span className="text-[16px] font-bold text-[#2BA76F] ml-1">
                        đ
                      </span>
                    </div>
                  </div>
                ...
                <div className="bg-slate-100 p-4 rounded-[20px] border border-slate-200/60 mt-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[13px] font-bold text-slate-700">
                      Liên kết Ngân hàng (Tùy chọn)
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-500 font-medium mb-4 leading-relaxed">
                    Nếu liên kết, hệ thống sẽ tự động gợi ý Ví này khi bạn thực
                    hiện các giao dịch chuyển khoản phù hợp.
                  </p>

                  <div className="space-y-3">
                    <div>
                      <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wide mb-1.5 ml-1">
                        Ngân hàng
                      </label>
                      <select
                        value={bankBin}
