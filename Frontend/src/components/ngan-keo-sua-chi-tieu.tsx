...
export function EditExpenseDrawer({
    ...
    if (open) {
      const fetchExpense = async () => {
        try {
          ...
          const isAllSelected =
            members.length > 0 &&
            members.every((m) => splitIds.includes(m.user.id));
  ...
  const onSubmit = async (values: z.infer<typeof expenseSchema>) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(
        error.response?.data?.message || "Có lỗi xảy ra khi cập nhật khoản chi",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDelete = async () => {
    try {
      ...
      console.error(error);
      toast.error(
        error.response?.data?.message || "Có lỗi xảy ra khi xóa khoản chi",
      );
    } finally {
      setIsDeleting(false);
    }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
        ...
        {!isOwner && expenseInfo && (
          <div className="bg-gray-50 rounded-2xl p-4 text-center text-sm text-gray-600 space-y-1">
            ...
            <p className="text-2xl font-black text-[#27AE60]">
              {new Intl.NumberFormat("vi-VN").format(expenseInfo.amount)}đ
            </p>
            <p>
              Người trả:{" "}
            ...
            <p className="text-xs text-gray-400 mt-2">
              Bạn đang xem với tư cách thành viên được chia tiền
            </p>
          </div>
        )}
                ...
                {splitMode === "all" ? (
                  <div className="bg-[#f0f9f6] border border-[#d1f2e6] rounded-2xl p-4 text-center">
                    ...
                    <p className="text-xs text-[#3ca27d] mt-0.5 font-medium">
                      Hóa đơn này sẽ tự động chia đều cho tất cả{" "}
      ...
      <Dialog open={isConfirmDeleteOpen} onOpenChange={setIsConfirmDeleteOpen}>
        <DialogContent
          ...
          className="max-w-[270px] w-[85vw] bg-white border border-[#88c9ad] rounded-[20px] p-5 shadow-xl text-center"
        >
          <div className="flex justify-center mb-3">
            <div className="w-12 h-12 flex items-center justify-center transform -rotate-12">
              <svg
                className="w-12 h-12 fill-[#88c9ad]"
                viewBox="0 0 24 24"
