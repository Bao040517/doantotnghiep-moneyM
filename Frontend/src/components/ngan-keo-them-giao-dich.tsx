...
export function AddTransactionDrawer({
  ...
  const handleSubmit = async (e?: React.FormEvent, isQuickAdd = false) => {
    ...
    if (isQuickAdd) {
      const uncat = categories.find(c => c.name.toLowerCase() === "chưa phân loại" && c.type === type);
      if (!uncat) {
        toast.error(`Không tìm thấy danh mục "Chưa phân loại" (${type === 'INCOME' ? 'Thu' : 'Chi'}). Vui lòng tạo trước!`);
        return;
      }
      payloadCategoryId = uncat.id;
      // We will override note below in the API call
    } else {
      if (isSplit) {
        ...
        if (totalSplit !== rawAmount) {
          toast.error(
            `Tổng chia nhỏ (${totalSplit}) không khớp với tổng tiền (${rawAmount})`,
          );
          return;
        }
    ...
    try {
      if (isGroupExpense && selectedGroupId) {
        ...
        const categoryName =
          filteredCategories.find((c) => c.id === payloadCategoryId)?.name ||
          "Khác";
        ...
        await api.post(`/transactions/${walletId}`, {
          ...
          note: isQuickAdd ? "Giao dịch nhập nhanh" : (note || undefined),
        ...
        toast.success(
          type === "INCOME" ? "Đã thêm thu nhập! 🎉" : "Đã thêm chi tiêu!",
        );
      }
      onCreated();
      onOpenChange(false);
        ...
        <div className="mx-auto w-full max-w-sm">
          ...
          <div className="p-4 pb-0">
            ...
            >
              <div className="space-y-2">
                ...
                <div className="flex gap-2 relative">
                  <div className="relative flex-1">
                    <span
                      className={`absolute left-3 top-1/2 -translate-y-1/2 font-semibold ${type === "INCOME" ? "text-emerald-500" : "text-rose-500"}`}
                    >
                      {currency === "VND"
                        ? "₫"
                        : currency === "USD"
                  ...
                  <div
                    className={`relative w-24 border rounded-md overflow-hidden flex items-center ${type === "INCOME" ? "border-emerald-200 bg-emerald-50" : "border-rose-200 bg-rose-50"}`}
                  >
                    <select
                      value={currency}
                ...
                {currency !== "VND" && exchangeRate > 0 && amount && (
                  <p
                    className={`text-xs font-medium flex justify-end mt-1 ${type === "INCOME" ? "text-emerald-600" : "text-rose-600"}`}
                  >
                    ≈{" "}
                    {new Intl.NumberFormat("vi-VN").format(
                      Math.round(
                        Number(amount.replace(/\D/g, "")) * exchangeRate,
                      ),
                    )}{" "}
                    đ (1 {currency} ={" "}
                    {new Intl.NumberFormat("vi-VN").format(
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger className="w-full">
                      {categoryId ? (
                        (() => {
                          const c = filteredCategories.find(
              ...
              {!isSplit &&
                categoryId &&
                budgets.filter((b) => b.categoryId === categoryId).length >
                  0 && (
                  <div className="space-y-2">
                    <Label
                      htmlFor="linkedBill"
                    ...
                    <Select
                      ...
                      onValueChange={(val) =>
                        setLinkedBudgetId(val && val !== "none" ? val : "")
                      }
                    >
                      <SelectTrigger className="w-full">
                        {linkedBudgetId ? (
                          (() => {
                            const b = budgets.find(
                              (bud) => bud.budgetId === linkedBudgetId,
                            );
                            return b ? (
                              <span>{b.name}</span>
                            ) : (
                      ...
                      {new Intl.NumberFormat("vi-VN").format(
                        Math.max(
                          ...
                          Number(amount.replace(/\D/g, "")) -
                            splits.reduce(
                              (acc, s) =>
                                acc + Number(s.amount.replace(/\D/g, "")),
                              0,
                            ),
                        ),
                      )}
                ...
                <div className="space-y-4 pt-2 border-t border-gray-100">
                  <div className="flex items-center justify-between">
                    <Label className="text-sm font-semibold text-gray-700">
                      Chi hộ nhóm? (Tự động chia đều)
                    </Label>
                    <button
                      type="button"
                      onClick={() => {
                        setIsGroupExpense(!isGroupExpense);
                        setIsSplit(false);
                      }}
                    ...
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${isGroupExpense ? "translate-x-5" : "translate-x-0"}`}
                      />
                    </button>
                  </div>
                  ...
                  {isGroupExpense && (
                    <div className="space-y-2 mt-2">
                      ...
                      <p className="text-xs text-gray-500">
                        Mặc định chia đều cho tất cả thành viên trong nhóm.
                      </p>
                    </div>
                  )}
          ...
          <DrawerFooter className="flex gap-2 w-full">
            {!isGroupExpense && (
              <Button
                ...
                className="flex-1 py-6 text-sm font-bold border-dashed border-amber-400 text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-xl whitespace-pre-wrap leading-tight h-auto px-1"
              >
                ⚡ Nhập nhanh{"\n"}(Chưa phân loại)
              </Button>
            )}
            <Button
              ...
              disabled={loading}
              className={`${isGroupExpense ? "w-full" : "flex-[2]"} py-6 text-base font-bold shadow-md rounded-xl ${
                type === "INCOME"
