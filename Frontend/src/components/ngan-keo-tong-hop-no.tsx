...
export function GlobalDebtsDrawer({
  myDebts,
  owedToMe,
  defaultTab = "owedToMe",
  children,
                ...
                {owedToMe.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      Các khoản nợ đã được thanh toán hết.
                    </p>
                  </div>
                ) : (
                  owedToMe.map((t, idx) => (
                    <div
                      key={idx}
                      className="bg-slate-50 rounded-2xl p-3 border border-slate-100 flex items-center gap-3"
                    >
                      <Avatar className="h-12 w-12 border border-slate-200">
                        <AvatarImage
                          src={
                            t.from.avatarUrl ||
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(t.from.name)}`
                          }
                        />
                      ...
                      <div className="text-right">
                        ...
                        <button
                          onClick={() => handleNavigateToGroup(t.groupId)}
                          className="text-[10px] font-bold text-emerald-700 underline mt-0.5"
                        >
                          Vào nhóm
                        </button>
                      </div>
              ...
              <div className="overflow-y-auto max-h-[50vh] space-y-3">
                {myDebts.length === 0 ? (
                  ...
                  myDebts.map((t, idx) => (
                    <div
                      key={idx}
                      className="bg-rose-50/50 rounded-2xl p-3 border border-rose-100 flex items-center gap-3"
                    >
                      <Avatar className="h-12 w-12 border border-slate-200">
                        <AvatarImage
                          src={
                            t.to.avatarUrl ||
                            `https://ui-avatars.com/api/?name=${encodeURIComponent(t.to.name)}`
                          }
                        />
                      ...
                      <div className="text-right">
                        ...
                        <button
                          onClick={() => handleNavigateToGroup(t.groupId)}
                          className="text-[10px] font-bold text-rose-600 underline mt-0.5"
                        >
                          Vào nhóm trả nợ
                        </button>
                      </div>
