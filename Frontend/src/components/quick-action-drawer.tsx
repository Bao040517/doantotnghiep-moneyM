...
export function QuickActionDrawer({ open, onOpenChange, onSelectTxType, onSelectCreateGroup }: QuickActionDrawerProps) {
  if (!open) return null;

  return (
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={() => onOpenChange(false)}>
      <div className="bg-white w-full max-w-md rounded-t-[2rem] p-6 animate-in slide-in-from-bottom" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-3">
          <button
            onClick={() => { onSelectTxType("EXPENSE"); }}
        ...
        <button className="w-full mt-6 py-3 font-bold text-gray-500 hover:text-gray-700" onClick={() => onOpenChange(false)}>
          Đóng
        </button>
      </div>
    </div>
