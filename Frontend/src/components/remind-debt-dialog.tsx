...
export function RemindDebtDialog({ groupId, debtorId, debtorName, amount, children }: RemindDebtDialogProps) {
  ...
  const handleSendReminder = async () => {
    ...
    try {
      ...
      console.error(err);
      toast.error(err.response?.data?.message || "Có lỗi xảy ra khi gửi nhắc nợ.");
    } finally {
      setIsSending(false);
    }
  };
  ...
  const handleOpenChange = (newOpen: boolean) => {
    if (newOpen) {
      setMessage(`Ê ${debtorName}, mầy còn nợ tao ${formatCurrency(amount)} đó nha! Trả lẹ đi mậy!`);
    }
    setOpen(newOpen);
  };
      ...
      <DialogContent className="sm:max-w-md bg-white">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 flex gap-2 items-end">
            ...
            >
              {isGenerating ? "Đang nghĩ..." : <><Sparkles className="w-3 h-3 mr-1" /> Soạn văn</>}
            </Button>
          </div>
