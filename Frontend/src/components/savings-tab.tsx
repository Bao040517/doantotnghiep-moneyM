...
import { AddSavingsGoalDrawer } from "./add-savings-goal-drawer";
import { EditSavingsGoalDrawer } from "./edit-savings-goal-drawer";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const PRIORITY_LEVELS: Record<number, { label: string; icon: string; pct: number; color: string }> = {
  5: { label: "Tối quan trọng", icon: "🔥", pct: 50, color: "bg-rose-100 text-rose-700 shadow-[0_2px_10px_-3px_rgba(225,29,72,0.3)]" },
  4: { label: "Ưu tiên cao", icon: "⭐", pct: 30, color: "bg-orange-100 text-orange-700" },
  3: { label: "Bình thường", icon: "🌱", pct: 15, color: "bg-[#EAF9F1] text-[#1A342B]" },
  2: { label: "Thong thả", icon: "🐢", pct: 5, color: "bg-blue-100 text-blue-700" },
  1: { label: "Tạm ngưng", icon: "⏸️", pct: 0, color: "bg-gray-100 text-gray-500" },
};
...
export function SavingsTab() {
  ...
  const updateAllocation = (goalId: string, newLevel: number) => {
    ...
    if (typeof window !== 'undefined') localStorage.setItem('savingsAllocations', JSON.stringify(newAllocs));
  };
      ...
      <main className="px-5 mt-4 w-full">
        ...
        <section className="bg-white rounded-[24px] p-4 mb-6 shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-[15px] font-extrabold text-gray-800">Tổng tiền đã tiết kiệm</h2>
            <div className="w-8 h-8 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-500 shadow-sm border border-emerald-100">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M21 18v1c0 1.1-.9 2-2 2H5c-1.11 0-2-.9-2-2V5c0-1.1.89-2 2-2h14c1.1 0 2 .9 2 2v1h-9c-1.11 0-2 .9-2 2v8c0 1.1.89 2 2 2h9zm-9-2h10V8H12v8zm4-2.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"></path></svg>
            </div>
          </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full bg-[#45b39d] rounded-full transition-all duration-500" style={{ width: `${overallPercent}%` }}></div>
                  </div>
                  <span className="text-[11px] font-bold text-gray-500">{overallPercent}% mục tiêu chung</span>
                </div>
          ...
          <div className="pt-3 pb-2 border-t border-gray-50 flex justify-between items-center">
            ...
            <button onClick={() => setShowAddDrawer(true)} className="text-xs font-bold text-[#45b39d] hover:text-[#3a9885] transition-colors flex items-center active:scale-95">
        ...
        {/* ─── GOALS LIST ─── */}
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-[17px] font-extrabold text-gray-800">Mục tiêu của bạn</h3>
        </div>
        ...
        <div className="space-y-4">
          {(() => {
            ...
            let activeGoals = sortedGoals.map(goal => {
            ...
            const totalSuggested = Object.values(allocationsMap).reduce((a,b)=>a+b, 0);
            ...
            const displayGoals = [...sortedGoals].sort((a, b) => {
              ...
              const pctA = a.targetAmount > 0 ? Math.min(100, Math.round((allocA / a.targetAmount) * 100)) : 0;
              const pctB = b.targetAmount > 0 ? Math.min(100, Math.round((allocB / b.targetAmount) * 100)) : 0;
              return pctB - pctA;
            });
            ...
            return (
              <>
                {displayGoals.map((goal, idx) => {
                  ...
                  return (
                    <div key={goal.id} className="bg-white rounded-[24px] p-4 border border-gray-100 shadow-sm flex flex-col relative hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3 w-full pr-2">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-[#45b39d] mt-1 font-bold">Tích lũy: {formatCurrency(finalAllocated)}</p>
                            <p className="text-[10px] text-gray-400 mt-0.5">Mục tiêu: {formatCurrency(goal.targetAmount)}</p>
                          </div>
                        </div>
                        ...
                        <div className="relative shrink-0">
                          <DropdownMenu>
                            <DropdownMenuTrigger className={`px-2.5 py-1.5 text-[10px] font-bold border-none rounded-lg outline-none cursor-pointer ${PRIORITY_LEVELS[level].color} transition-colors flex items-center shadow-sm active:scale-95`}>
                              <span className="mr-1">{PRIORITY_LEVELS[level].icon}</span>
                              {PRIORITY_LEVELS[level].label}
                            </DropdownMenuTrigger>
                            <DropdownMenuContent align="end" className="w-48 rounded-[20px] p-2 bg-white/95 backdrop-blur-xl border border-gray-100 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.1)]">
                              {[5, 4, 3, 2, 1].map(lvl => (
                                <DropdownMenuItem 
                                  key={lvl}
                                ...
                                >
                                  <span className="text-lg">{PRIORITY_LEVELS[lvl].icon}</span>
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] font-bold text-[#45b39d]">{percent}% hoàn thành</span>
                        </div>
                        <button 
                          onClick={() => {
          ...
          {goals.length === 0 && (
            <div className="text-center py-10 opacity-60">
              ...
              <p className="font-semibold text-gray-700">Chưa có mục tiêu tiết kiệm nào</p>
