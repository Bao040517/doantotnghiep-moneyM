...
export function HistoryTab() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      const [txRes, summaryRes, debtRes] = await Promise.all([
        api.get(
          `/transactions/monthly?year=${selectedYear}&month=${selectedMonth}`,
        ),
        api.get(
        ...
        api
          ...
          .catch(() => ({ data: { totalOwed: 0, totalOwing: 0 } })),
      ]);
  ...
  const personalTransactions = transactions.filter(
    (t) => t.category?.name !== "Cho nhóm mượn",
  );
  ...
  const filteredTransactions = personalTransactions.filter(
    (t) =>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="border border-gray-100 rounded-2xl p-3">
            ...
            <div className="flex items-center text-sm font-bold text-blue-500">
              ...
              {new Intl.NumberFormat("vi-VN").format(
                totalPersonalIncome + debtSummary.totalOwed,
              )}
              đ
            </div>
          </div>
          ...
          <div className="border border-gray-100 rounded-2xl p-3">
            ...
            <div className="flex items-center text-sm font-bold text-amber-500">
              -
              {new Intl.NumberFormat("vi-VN").format(
                totalPersonalExpense + debtSummary.totalOwing,
              )}
              đ
            </div>
          </div>
          ...
          <div className="flex flex-col">
            {filteredTransactions.map((tx, idx) => {
              ...
              return (
                <div
                  key={tx.id}
                  className={`p-4 flex items-start gap-4 ${!isLast ? "border-b border-gray-50" : ""} hover:bg-gray-50 transition-colors`}
                >
                  {/* Icon */}
                  <div
                    className={`w-11 h-11 rounded-full flex items-center justify-center shrink-0 shadow-sm ${
                      tx.type === "INCOME"
                        ? "bg-[#e0f2fe] text-blue-500"
                        : "bg-[#fff7ed] text-orange-400"
                    }`}
                  >
                    {tx.category?.iconName ? (
                      <DynamicIcon
                        name={tx.category.iconName}
                        className="w-5 h-5"
                      />
                    ) : tx.type === "INCOME" ? (
                      "💵"
                  ...
                  <div className="text-right pl-2">
                    <p
                      className={`text-[15px] font-bold ${tx.type === "INCOME" ? "text-emerald-500" : "text-gray-800"}`}
                    >
                      {tx.type === "INCOME" ? "+" : "-"}
