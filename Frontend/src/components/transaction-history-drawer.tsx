...
export function TransactionHistoryDrawer({ open, onOpenChange, transactions }: TransactionHistoryDrawerProps) {
  return (
      ...
      <DrawerContent className="bg-[#f8fafc] max-h-[85vh]">
        <div className="mx-auto w-full max-w-md h-full flex flex-col">
          <DrawerHeader className="bg-white border-b border-gray-100 shrink-0">
            <DrawerTitle className="text-xl font-bold text-center">Lịch sử giao dịch</DrawerTitle>
            ...
            <div className="space-y-3">
              {transactions.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-[2rem] border border-dashed border-emerald-200">
                  ...
                  <p className="text-xs text-gray-400 mt-1">Tháng này chưa ghi chép giao dịch nào!</p>
                </div>
              ) : (
                transactions.map(tx => (
                  <div
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-sm ${
                        tx.type === "INCOME" ? "bg-[#d1fae5] text-emerald-600" : "bg-red-50 text-red-500"
                      }`}>
                        {tx.category?.iconName || (tx.type === "INCOME" ? "💵" : "💸")}
                      </div>
                      <div>
                        <p className="font-bold text-gray-800 text-[15px] leading-tight">{tx.category?.name || "Chi tiêu"}</p>
                        <p className="text-[11px] text-gray-400 mt-0.5">
                          {format(new Date(tx.transactionDate), "dd MMM, HH:mm", { locale: vi })}
                        </p>
                        {tx.note && (
                          <p className="text-[11px] text-gray-500 mt-0.5 truncate max-w-[160px]">{tx.note}</p>
                        )}
                        {tx.linkedExpenseId && (
                    ...
                    <div className="text-right">
                      <p className={`font-black text-base ${tx.type === "INCOME" ? "text-emerald-600" : "text-gray-900"}`}>
