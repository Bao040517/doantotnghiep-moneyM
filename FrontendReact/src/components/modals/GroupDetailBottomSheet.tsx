...
export const GroupDetailBottomSheet: React.FC<GroupDetailBottomSheetProps> = ({
  ...
  const handleAutoFillRemainder = () => {
    ...
    if (targetIds.length === 0) {
      Alert.alert("Thông báo", "Vui lòng chọn ít nhất 1 người tham gia");
      return;
    }
  ...
  const handleSaveExpense = async () => {
    ...
    if (splitType === "EQUAL") {
      ...
      if (selectedSplitUserIds.length === 0 || targetSplitIds.length === 0) {
        Alert.alert("Lỗi", "Vui lòng chọn người tham gia và nhập số tiền lớn hơn 0");
        return;
      }

      if (sum !== rawNumber) {
        Alert.alert("Lỗi", `Tổng chia (${sum.toLocaleString("vi-VN")} ₫) chưa khớp hóa đơn (${rawNumber.toLocaleString("vi-VN")} ₫)`);
        return;
      }
    }
                        ...
                        onPress={() => {
                          if (isSelected) {
                            if (selectedSplitUserIds.length > 1) {
                              ...
                              Alert.alert("Thông báo", "Phải có ít nhất 1 người chia tiền");
                            }
                          } else {
                            setSelectedSplitUserIds([...selectedSplitUserIds, uId]);
                          }
