...
let sql = `-- ============================================================================
-- SHAREMONEY DATABASE SEED SCRIPT - VERSION 12 (SEED_V12.SQL)
-- Phiên bản nâng cấp tối ưu: Tái cân bằng ngân sách toàn diện, Cân bằng xám, Giới hạn <= 2Tr
-- Tài khoản <= 25Tr, Đồng bộ 100% Entity Spring Boot, Đa Cổng Thanh Toán (PayOS / VNPay)
...
users.forEach(u => {
    ...
    u.years.forEach(year => {
        ...
        for (let m = startM; m <= endM; m++) {
            ...
            // 4. Fixed Monthly Expenses (Rent, Elec, Telecom) for past months
            if (year < 2026 || m <= 7) {
            ...
            // ══════════════════════════════════════════════════════════════════════
            // SPECIAL SETUP FOR CURRENT MONTH (8/2026) - PERFECT REBALANCE TEST DATA
            // ══════════════════════════════════════════════════════════════════════
            if (year === 2026 && m === 8) {
                // 1. Khoản Cố định vượt hạn mức: Tiền nhà (Hạn mức 1.800.000đ, Thực chi 1.850.000đ -> Tiêu lố 50.000đ)
                const rentTxId = uuid();
                const rentTxDate = `2026-08-04 10:00:00`;
                state.txs.push(`('${rentTxId}', '${wMain}', 1850000, 'EXPENSE', '${categoryMap['Tiền nhà']}', '${budgetIdMap['Tiền nhà']}', '${pArr[0]}', '${rentTxDate}', 'Tiền phòng trọ và phụ phí quản lý T8', false, false, false, '${rentTxDate}')`);
                netMainBalance -= 1850000;

                // 2. Khoản Linh hoạt vượt hạn mức: Ăn uống (Hạn mức 2.000.000đ, Thực chi 2.150.000đ -> Tiêu lố 150.000đ)
                const foodTx1 = uuid();
                state.txs.push(`('${foodTx1}', '${wMain}', 650000, 'EXPENSE', '${categoryMap['Ăn uống']}', '${budgetIdMap['Ăn uống']}', '${pArr[3]}', '2026-08-02 12:30:00', 'Đi siêu thị mua đồ ăn tuần 1', false, false, false, '2026-08-02 12:30:00')`);
                const foodTx2 = uuid();
                state.txs.push(`('${foodTx2}', '${wMain}', 1500000, 'EXPENSE', '${categoryMap['Ăn uống']}', '${budgetIdMap['Ăn uống']}', '${pArr[3]}', '2026-08-08 19:30:00', 'Tiệc buffet lẩu nướng sinh nhật', false, false, false, '2026-08-08 19:30:00')`);
                netMainBalance -= 2150000;

                // 3. Khoản Tier 1 Luxury còn dư dồi dào để đề xuất Cắt giảm bù trừ: Quần áo (Hạn mức 1.000.000đ, Thực chi 300.000đ -> Còn dư 700.000đ -> Đề xuất cắt 200.000đ xuống 800.000đ)
                const clothTx = uuid();
                state.txs.push(`('${clothTx}', '${wMain}', 300000, 'EXPENSE', '${categoryMap['Quần áo']}', '${budgetIdMap['Quần áo']}', '${pArr[5]}', '2026-08-03 15:00:00', 'Mua áo thun cotton Uniqlo', false, false, false, '2026-08-03 15:00:00')`);
                netMainBalance -= 300000;

                // 4. Khoản đã cân bằng: Chi tiêu hàng ngày (Hạn mức 1.500.000đ, Thực chi 1.480.000đ -> Còn dư 20.000đ <= 30k -> Thẻ xám Đã cân bằng)
                const dailyTx = uuid();
                state.txs.push(`('${dailyTx}', '${wMain}', 1480000, 'EXPENSE', '${categoryMap['Chi tiêu hàng ngày']}', '${budgetIdMap['Chi tiêu hàng ngày']}', '${pArr[3]}', '2026-08-06 18:00:00', 'Mua đồ gia dụng và nhu yếu phẩm tháng 8', false, false, false, '2026-08-06 18:00:00')`);
                netMainBalance -= 1480000;

                // 5. Khoản tiệm cận: Phí giao lưu (Hạn mức 1.500.000đ, Thực chi 1.280.000đ -> 85.3% -> Thẻ xám Đã cân bằng)
                const socialTx = uuid();
                state.txs.push(`('${socialTx}', '${wMain}', 1280000, 'EXPENSE', '${categoryMap['Phí giao lưu']}', '${budgetIdMap['Phí giao lưu']}', '${pArr[1]}', '2026-08-07 14:00:00', 'Mừng tân gia đồng nghiệp phòng IT', false, false, false, '2026-08-07 14:00:00')`);
                netMainBalance -= 1280000;

                // 6. Khoản đã cân bằng: Đi lại (Hạn mức 800.000đ, Thực chi 780.000đ -> Còn dư 20.000đ -> Thẻ xám Đã cân bằng)
                const transTx = uuid();
                state.txs.push(`('${transTx}', '${wMain}', 780000, 'EXPENSE', '${categoryMap['Đi lại']}', '${budgetIdMap['Đi lại']}', '${pArr[0]}', '2026-08-09 08:00:00', 'Nạp thẻ giao thông & Grab đi làm đầu tháng', false, false, false, '2026-08-09 08:00:00')`);
                netMainBalance -= 780000;

                // 7. Khoản Cố định an toàn: Tiền điện (Hạn mức 750.000đ, Thực chi 620.000đ)
                const elecTx = uuid();
                state.txs.push(`('${elecTx}', '${wMain}', 620000, 'EXPENSE', '${categoryMap['Tiền điện']}', '${budgetIdMap['Tiền điện']}', '${pArr[1]}', '2026-08-10 14:00:00', 'Thanh toán tiền điện EVN T8', false, false, false, '2026-08-10 14:00:00')`);
                netMainBalance -= 620000;

                // 8. Khoản Cố định an toàn: Phí liên lạc (Hạn mức 200.000đ, Thực chi 90.000đ)
                const phoneTx = uuid();
                state.txs.push(`('${phoneTx}', '${wMain}', 90000, 'EXPENSE', '${categoryMap['Phí liên lạc']}', '${budgetIdMap['Phí liên lạc']}', '${pArr[2]}', '2026-08-11 09:30:00', 'Nạp gói cước 4G Viettel tháng 8', false, false, false, '2026-08-11 09:30:00')`);
                netMainBalance -= 90000;

                // Multi-Gateway payment orders for User A in current month (8/2026)
                if (u.id === users[0].id) {
...
fs.writeFileSync('seed_v12.sql', sql, 'utf8');
console.log('Successfully generated seed_v12.sql with exact Rebalance & Compensation cuts test data, max budget <= 2,000,000 VND, max wallet balance <= 25,000,000 VND, and 100% Entity parity!');
