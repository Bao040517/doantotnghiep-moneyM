...
let sql = `-- ============================================================================
-- SHAREMONEY DATABASE SEED SCRIPT - VERSION 14 (SEED_V14.SQL)
...
-- Đầy đủ các khoản Ngân sách BILL (Hóa đơn cố định) liên kết STK người nhận sẵn sàng test luồng Trả ngay VietQR/PayOS
-- ============================================================================
...
users.forEach(u => {
    ...
    if (u.id === users[0].id) {
        ...
        userPayeeList.push(
            ...
            { name: 'Ban Quản Lý Chung Cư Sky Garden', bankBin: '970426', bankName: 'MSB', bankAccount: '4517052004999', accountName: 'BQL CHUNG CU SKY', phone: '02839998888' },
            { name: 'Công ty Tài chính FE Credit', bankBin: '970432', bankName: 'VPBank', bankAccount: '999888777666', accountName: 'FE CREDIT VIETNAM', phone: '19002345' }
        );
    ...
    u.years.forEach(year => {
        ...
        for (let m = startM; m <= endM; m++) {
            ...
            // 3. Budgets for this month - SPECIFICALLY MANY BILL BUDGETS FOR USERS A, B, C
            const budgetIdMap = {};
            const bConfigs = [];

            if (u.id === users[0].id) {
                // USER A: 4 BILL BUDGETS (Tiền nhà -> B Techcombank, Tiền điện -> EVN, Phí liên lạc -> Viettel, Nước/Chung cư -> C MSB)
                bConfigs.push(
                ...
                // USER B: 4 BILL BUDGETS (Tiền nhà -> C MSB, Tiền điện -> EVN, Phí liên lạc -> Viettel, Trả tiền phòng -> A MBBank)
                bConfigs.push(
                ...
                // USER C: 4 BILL BUDGETS (Tiền nhà -> A MBBank, Tiền điện -> EVN, Phí liên lạc -> Viettel, Học phí/Quỹ -> B Techcombank)
                bConfigs.push(
            ...
            if (year < 2026 || m <= 7) {
                ...
                if (u.id === users[0].id) {
                    ...
                    // 3. Phí liên lạc: CHƯA TRẢ ĐỒNG NÀO (0 / 250k -> TEST TRẢ NGAY 1-TOUCH CHO VIETTEL)

                    // 4. Ăn uống: Overspent 150k (2.15tr / 2tr)
                    ...
                    // 3. Phí liên lạc: CHƯA TRẢ (0 / 200k -> TEST TRẢ NGAY)
                    ...
                    // 3. Phí liên lạc: CHƯA TRẢ (0 / 220k -> TEST TRẢ NGAY)
...
fs.writeFileSync('seed_v14.sql', sql, 'utf8');
console.log(`Successfully generated seed_v14.sql: ${state.txs.length} transactions, ${state.expenses.length} group expenses, ${state.budgets.length} budgets strictly bounded up to 20/08/2026!`);
