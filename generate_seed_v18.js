...
-- Bugfixes from V17:
--   ✅ FIX: Schema DEFAULT 'DYNAMIC' → 'FLEXIBLE' (prevents JPA BudgetType crash)
--   ✅ FIX: Added migration UPDATE to convert any existing 'DYNAMIC' rows
--   ✅ FIX: ALTER COLUMN type SET DEFAULT 'FLEXIBLE' for existing tables
-- Features:
...
CREATE TABLE IF NOT EXISTS budgets (
    ...
    type VARCHAR(50) NOT NULL DEFAULT 'FLEXIBLE',
...
UPDATE budgets SET type = 'FLEXIBLE' WHERE type = 'DYNAMIC';
...
fs.writeFileSync('seed_v18.sql', fullSql, 'utf8');

const sizeKB = Math.round(fullSql.length / 1024);
console.log(`✅ Successfully generated seed_v18.sql (${sizeKB} KB, ${fullSql.split('\n').length} lines)!`);
console.log(`   Schema DEFAULT: FLEXIBLE (NO MORE DYNAMIC)`);
console.log(`   Migration: UPDATE budgets SET type = 'FLEXIBLE' WHERE type = 'DYNAMIC'`);
