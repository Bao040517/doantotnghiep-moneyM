...
// Generates dates strictly within target month
function randomDateInMonth(year, month) {
    const maxDay = (month === 2) ? 28 : (month === 4 || month === 6 ? 30 : 28); // Cap at 28 for safe day bounds
...
const users = [
    { id: '1a111111-1111-4111-a111-111111111111', name: 'Nguyễn Văn A (Thông Thái)', email: 'nguyenvana@gmail.com', avatar: 'https://ui-avatars.com/api/?name=A&background=0D8ABC&color=fff', baseSalary: 32000000, rentAmt: 6000000, persona: 'SAVER' },
    { id: '1b111111-1111-4111-a111-111111111111', name: 'Trần Thị B (Tiêu Lố)', email: 'tranthib@gmail.com', avatar: 'https://ui-avatars.com/api/?name=B&background=10B981&color=fff', baseSalary: 18000000, rentAmt: 4500000, persona: 'SPENDER' },
    { id: '1c111111-1111-4111-a111-111111111111', name: 'Lê Thị C (Trùm Nhóm)', email: 'lethic@gmail.com', avatar: 'https://ui-avatars.com/api/?name=C&background=F43F5E&color=fff', baseSalary: 25000000, rentAmt: 5000000, persona: 'GROUP_LEADER' },
    { id: '1d111111-1111-4111-a111-111111111111', name: 'Phạm Văn D (Con Nợ)', email: 'phamvand@gmail.com', avatar: 'https://ui-avatars.com/api/?name=D&background=8B5CF6&color=fff', baseSalary: 14000000, rentAmt: 3500000, persona: 'DEBTOR' },
    { id: '1e111111-1111-4111-a111-111111111111', name: 'Hoàng Thị E (Newbie GenZ)', email: 'hoangthie@gmail.com', avatar: 'https://ui-avatars.com/api/?name=E&background=EC4899&color=fff', baseSalary: 9500000, rentAmt: 2500000, persona: 'NEWBIE' }
];
...
const groceryItems = [
    ...
    { name: 'CoopMart - Mua đồ dùng gia đình', min: 35000, max: 850000 },
...
let sql = `-- ============================================================================
-- SHAREMONEY DATABASE SEED V4 (2026-01-01 to 2026-08-31 - 8 Full Months)
...
const userValues = users.map((u, i) => `('${u.id}', '${u.email}', '$2a$10$GK1LUpu5xnOCQt1I4V5zz.A4crOZWPjtcC3CHmaaZoqJitEgxpFXm', '${u.name}', '090${i}123456', '${u.avatar}', '970422', '109${i}8888999', '2026-01-01 08:00:00')`);
sql += userValues.join(',\n') + ';\n\n';
...
// Global Group ID for group expense testing
...
state.groups.push(`('${groupDinnerId}', 'Hội Ăn Nhậu Cuối Tuần', 'Quỹ ăn uống gộp', '${users[0].id}', '2026-01-01 08:00:00')`);

users.forEach(u => {
    state.groupMembers.push(`('${uuid()}', '${groupTripId}', '${u.id}', '${u.id === users[2].id ? 'owner' : 'member'}', '2026-01-01 08:00:00')`);
    state.groupMembers.push(`('${uuid()}', '${groupDinnerId}', '${u.id}', '${u.id === users[0].id ? 'owner' : 'member'}', '2026-01-01 08:00:00')`);
});

users.forEach(u => {
    const w1 = uuid(); const w2 = uuid();
    state.wallets.push(`('${w1}', '${u.id}', 'Ví Tiền Mặt & MBBank', 45000000, 'VND', false, '2026-01-01 08:00:00')`);
    state.wallets.push(`('${w2}', '${u.id}', 'Thẻ Tín Dụng VPBank', -4500000, 'VND', true, '2026-01-01 08:00:00')`);
    ...
    if (u.persona === 'DEBTOR') {
        ...
        state.loans.push(`('${uuid()}', '${u.id}', 'LENT', 'Đồng nghiệp công ty', 4000000, 0, '2026-03-01', '2026-09-01', 'Cho mượn mua điện thoại', false, '2026-03-01 10:00:00')`);
    }

    // Savings Goals simulation
    const sAmt1 = u.persona === 'SAVER' ? 35000000 : 12000000;
    const sAmt2 = u.persona === 'SAVER' ? 25000000 : 5000000;
    state.savings.push(`('${uuid()}', '${u.id}', 'Quỹ Mua Xe Máy Mới', 50000000, ${sAmt1}, '2027-06-30', 'IN_PROGRESS', '2026-01-01 08:00:00')`);
    state.savings.push(`('${uuid()}', '${u.id}', 'Quỹ Dự Phòng Khẩn Cấp', 30000000, ${sAmt2}, '2026-12-31', 'IN_PROGRESS', '2026-01-01 08:00:00')`);
    ...
    // Budget configurations per month (Flexibility & Spender logic)
    const budgetMultiplier = (u.persona === 'SPENDER') ? 0.7 : 1.0; // Spender sets tight budgets but exceeds them
    const budgetConfigs = [
    ...
    // Loop 8 months (January 2026 to August 2026)
    for (let m = 1; m <= 8; m++) {
        ...
        const salaryDate = `${2026}-${mStr}-05 08:30:00`;
        const salaryCatId = categoryMap['Tiền lương'];
        state.txs.push(`('${salaryTxId}', '${w1}', ${u.baseSalary}, 'INCOME', '${salaryCatId}', '${pArr[4]}', '${salaryDate}', 'Lương tháng ${mStr}/2026 - Công ty Tech', false, false, false, '${salaryDate}')`);
        state.txTags.push(`('${salaryTxId}', '${tArr[3]}')`);

        // Quarterly Bonus on April & August (Day 25)
        if (m === 4 || m === 8) {
            const bonusTxId = uuid();
            const bonusDate = `${2026}-${mStr}-25 14:00:00`;
            const bonusAmt = Math.round(u.baseSalary * 0.4);
            state.txs.push(`('${bonusTxId}', '${w1}', ${bonusAmt}, 'INCOME', '${categoryMap['Tiền thưởng']}', '${pArr[4]}', '${bonusDate}', 'Thưởng KPI Quý ${m === 4 ? 1 : 2}', false, false, false, '${bonusDate}')`);
        ...
        // Rent
        const rentTxId = uuid();
        const rentDate = `${2026}-${mStr}-02 10:00:00`;
        state.txs.push(`('${rentTxId}', '${w1}', ${u.rentAmt}, 'EXPENSE', '${categoryMap['Tiền nhà']}', '${pArr[1]}', '${rentDate}', 'Tiền thuê nhà tháng ${mStr}/2026', false, false, false, '${rentDate}')`);
        state.txTags.push(`('${rentTxId}', '${tArr[5]}')`);

        // Electricity EVN
        const elecTxId = uuid();
        const elecDate = `${2026}-${mStr}-08 15:30:00`;
        const elecAmt = randomInt(600, 1800) * 1000;
        state.txs.push(`('${elecTxId}', '${w1}', ${elecAmt}, 'EXPENSE', '${categoryMap['Tiền điện']}', '${pArr[6]}', '${elecDate}', 'Hóa đơn Điện lực EVN T${mStr}', false, false, false, '${elecDate}')`);

        // Internet Viettel
        const netTxId = uuid();
        const netDate = `${2026}-${mStr}-09 11:00:00`;
        state.txs.push(`('${netTxId}', '${w1}', 275000, 'EXPENSE', '${categoryMap['Phí liên lạc']}', '${pArr[7]}', '${netDate}', 'Cước Internet Viettel Fiber T${mStr}', false, false, false, '${netDate}')`);

        // 4. DAILY REALISTIC TRANSACTIONS (High density: ~50-70 txs/month)
        const daysInMonth = (m === 2) ? 28 : (m === 4 || m === 6 ? 30 : 28);
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            if (Math.random() > 0.15) {
                ...
                const foodDate = formatDate(2026, m, d, randomInt(7, 19), randomInt(0, 59));
            ...
            if (Math.random() > 0.35) {
                ...
                const drinkDate = formatDate(2026, m, d, randomInt(8, 16), randomInt(0, 59));
            ...
            if (Math.random() > 0.4) {
                ...
                const transDate = formatDate(2026, m, d, randomInt(7, 21), randomInt(0, 59));
            ...
            if (d % 3 === 0) {
                ...
                const grocDate = formatDate(2026, m, d, 18, randomInt(10, 50));
            ...
            if (Math.random() < shopChance) {
                ...
                const shopDate = formatDate(2026, m, d, 21, randomInt(0, 59));
        ...
        if (u.persona === 'SAVER') {
            const saveTxId = uuid();
            const saveDate = `${2026}-${mStr}-06 10:00:00`;
            const saveAmt = 5000000;
            state.txs.push(`('${saveTxId}', '${w1}', ${saveAmt}, 'EXPENSE', '${categoryMap['Mục tiêu tiết kiệm']}', '${pArr[1]}', '${saveDate}', 'Trích nạp Quỹ mua xe máy T${mStr}', false, false, false, '${saveDate}')`);
        ...
        if (u.id === users[2].id) { // Group leader pays
            const expId = uuid();
            const expDate = `${2026}-${mStr}-18 19:30:00`;
            const totalGroupAmt = 4000000;
            const perUserAmt = 800000;
            state.expenses.push(`('${expId}', '${groupDinnerId}', '${u.id}', 'Ăn tiệc lẩu nướng Gogi T${mStr}', ${totalGroupAmt}, 'Ăn uống', '${expDate}')`);
            ...
            users.forEach(gUser => {
                ...
                if (gUser.id !== u.id && isPaid) {
                    const payDate = `${2026}-${mStr}-19 10:00:00`;
...
fs.writeFileSync('seed_v4.sql', sql, 'utf8');
console.log('Successfully generated seed_v4.sql with high-density realistic Vietnamese financial data!');
