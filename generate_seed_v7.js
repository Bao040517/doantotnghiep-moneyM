...
let sql = `-- ============================================================================
-- SHAREMONEY DATABASE SEED V7 (COMPLETE ENTITY DATA & AUDIT NOTIFICATIONS)
        ...
        for (let m = 1; m <= 12; m++) {
            ...
            budgetConfigs.forEach(bConfig => {
                ...
                if (catId) {
                    ...
                    state.budgets.push(`('${uuid()}', '${u.id}', '${catId}', '${cleanBudgetName}', ${bConfig.limit}, ${m}, ${year}, '${bConfig.type}', true, ${bConfig.mandatory}, ${bConfig.dueDay}, '${bConfig.bin}', '${bConfig.acc}', '${bConfig.accName}', '${year}-${mStr}-01 08:00:00')`);
                }
            });
            ...
            if (year === 2026 && m === 8) {
                ...
                state.txs.push(`('${overRentId}', '${wMain}', 7341756, 'EXPENSE', '${categoryMap['Tiền nhà']}', '${pArr[0]}', '${overRentDate}', 'Tiền phòng trọ và phụ phí quản lý T8', false, false, false, '${overRentDate}')`);
...
fs.writeFileSync('seed_v7.sql', sql, 'utf8');
console.log('Successfully generated seed_v7.sql with clean budget names, full entity data and complete audit notifications!');
