...
let sql = `-- ============================================================================
-- SHAREMONEY DATABASE SEED SCRIPT - VERSION 15 (SEED_V15.SQL)
-- Dữ liệu 20 tháng (01/2025 -> 20/08/2026), Mốc thời gian thực tế cắt tại 20/08/2026
-- 3 User Đặc Biệt:
--   1. nguyenvana@gmail.com -> MBBank (970422) - STK: 6617052004888 - DUONG DUC BAO
--   2. nguyenvanb@gmail.com -> Techcombank (970407) - STK: 6617052004 - NGUYEN VAN B
--   3. nguyenvanc@gmail.com -> MSB (970426) - STK: 4517052004 - NGUYEN VAN C
-- ĐẶC BIỆT: 3 User A, B, C có NHIỀU KHOẢN NGÂN SÁCH BILL (Hóa đơn cố định: Tiền nhà, Tiền điện, Internet 4G, Học phí, Bảo hiểm Y tế)
-- có sẵn thông tin STK thụ hưởng để phục vụ test trọn vẹn luồng thanh toán Trả ngay VietQR P2P & PayOS
-- ============================================================================
...
users.forEach(u => {
    ...
    const userPayeeList = [];
    if (u.id === users[0].id) {
        // User A's Payees (Include User B Techcombank & User C MSB)
        userPayeeList.push(
        ...
        // User B's Payees (Include User A MBBank & User C MSB)
        userPayeeList.push(
        ...
        // User C's Payees (Include User A MBBank & User B Techcombank)
        userPayeeList.push(
    ...
    u.years.forEach(year => {
        ...
        for (let m = startM; m <= endM; m++) {
            ...
            // 3. Budgets for this month - 5 BILL BUDGETS (HÓA ĐƠN CỐ ĐỊNH) CHO USER A, B, C
            const budgetIdMap = {};
            const bConfigs = [];

            if (u.id === users[0].id) {
                // USER A: 5 BILL BUDGETS (Tiền nhà -> B Techcombank, Tiền điện -> EVN, Phí liên lạc -> Viettel, Học phí -> FPT, Y tế -> Bảo Việt)
                bConfigs.push(
                ...
                // USER B: 5 BILL BUDGETS (Tiền nhà -> C MSB, Tiền điện -> EVN, Phí liên lạc -> Viettel, Tiền phòng trả A -> A MBBank, Y tế)
                bConfigs.push(
                ...
                // USER C: 5 BILL BUDGETS (Tiền nhà -> A MBBank, Tiền điện -> EVN, Phí liên lạc -> Viettel, Học phí -> VUS, Y tế -> CarePlus)
                bConfigs.push(
            ...
            if (year < 2026 || m <= 7) {
                ...
                if (u.id === users[0].id) {
                    ...
                    // 3. Phí liên lạc: CHƯA TRẢ (0 / 250k -> TEST TRẢ NGAY 1-TOUCH CHO VIETTEL)
                    // 4. Giáo dục: CHƯA TRẢ (0 / 1.2tr -> TEST TRẢ NGAY HỌC PHÍ FPT)
                    // 5. Y tế: CHƯA TRẢ (0 / 500k -> TEST TRẢ NGAY BẢO HIỂM BẢO VIỆT)

                    // 6. Ăn uống: Overspent 150k (2.15tr / 2tr)
                    ...
                    // 3. Phí liên lạc: CHƯA TRẢ (0 / 200k -> TEST TRẢ NGAY VIETTEL)
                    // 4. Giáo dục: CHƯA TRẢ (0 / 800k -> TEST TRẢ NGAY FPT TELECOM)
                    // 5. Y tế: CHƯA TRẢ (0 / 400k -> TEST TRẢ NGAY HOÀN MỸ)
                    ...
                    // 3. Phí liên lạc: CHƯA TRẢ (0 / 220k -> TEST TRẢ NGAY VIETTEL)
                    // 4. Giáo dục: CHƯA TRẢ (0 / 1tr -> TEST TRẢ NGAY VUS)
                    // 5. Y tế: CHƯA TRẢ (0 / 450k -> TEST TRẢ NGAY CAREPLUS)
...
fs.writeFileSync('seed_v15.sql', sql, 'utf8');
console.log(`Successfully generated seed_v15.sql: ${state.txs.length} transactions, ${state.expenses.length} group expenses, ${state.budgets.length} budgets strictly bounded up to 20/08/2026!`);
