...
public class SavingsGoalController {
    ...
    public ResponseEntity<List<SavingsGoalResponse>> getUserSavingsGoals() {
        return ResponseEntity.ok(
                savingsGoalService.getUserSavingsGoals(SecurityUtils.getCurrentUserId()));
    }
    ...
    public ResponseEntity<SavingsGoalResponse> createSavingsGoal(
        ...
        return ResponseEntity.ok(
                savingsGoalService.createSavingsGoal(SecurityUtils.getCurrentUserId(), request));
    }

    @PutMapping("/{goalId}")
    public ResponseEntity<SavingsGoalResponse> updateSavingsGoal(
            @PathVariable UUID goalId, @Valid @RequestBody SavingsGoalRequest request) {
        return ResponseEntity.ok(
                savingsGoalService.updateSavingsGoal(
                        SecurityUtils.getCurrentUserId(), goalId, request));
    }

    @PostMapping("/{goalId}/fund")
    public ResponseEntity<SavingsGoalResponse> fundSavingsGoal(
            @PathVariable UUID goalId, @Valid @RequestBody FundSavingsGoalRequest request) {
        return ResponseEntity.ok(
                savingsGoalService.fundSavingsGoal(
                        SecurityUtils.getCurrentUserId(), goalId, request));
    }

    @PostMapping("/{goalId}/withdraw")
    public ResponseEntity<SavingsGoalResponse> withdrawSavingsGoal(
            @PathVariable UUID goalId, @Valid @RequestBody WithdrawSavingsGoalRequest request) {
        return ResponseEntity.ok(
                savingsGoalService.withdrawSavingsGoal(
                        SecurityUtils.getCurrentUserId(), goalId, request));
    }
