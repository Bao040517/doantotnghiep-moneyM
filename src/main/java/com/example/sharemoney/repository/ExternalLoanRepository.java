...
public interface ExternalLoanRepository extends JpaRepository<ExternalLoan, UUID> {
    ...
    @Query(
            "SELECT SUM(e.principalAmount) FROM ExternalLoan e WHERE e.user.id = :userId AND e.type = :type AND e.isSettled = false")
    BigDecimal sumUnsettledAmountByUserIdAndType(
            @Param("userId") UUID userId, @Param("type") LoanType type);
}
