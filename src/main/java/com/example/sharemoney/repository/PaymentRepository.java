...
public interface PaymentRepository extends JpaRepository<Payment, UUID> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.receiver.id = :userId AND p.status = :status")
    java.math.BigDecimal sumAmountByReceiverIdAndStatus(@org.springframework.data.repository.query.Param("userId") UUID userId, @org.springframework.data.repository.query.Param("status") String status);

    @org.springframework.data.jpa.repository.Query("SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.payer.id = :userId AND p.status = :status")
    java.math.BigDecimal sumAmountByPayerIdAndStatus(@org.springframework.data.repository.query.Param("userId") UUID userId, @org.springframework.data.repository.query.Param("status") String status);
