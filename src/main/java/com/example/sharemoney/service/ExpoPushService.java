...
public class ExpoPushService {
    ...
    /**
     * Bắn Native Push Notification ra màn hình khóa của điện thoại thông qua Expo Push API (APNs / FCM).
     * Chạy bất đồng bộ để đảm bảo tốc độ phản hồi API 0ms cho client.
     */
    @Async
    public void sendPushNotification(String pushToken, String title, String body, Map<String, Object> data) {
        ...
        try {
            ...
            log.info("[EXPO PUSH SUCCESS] Đã gửi thông báo tới {}: Title='{}', Status={}", pushToken, title, response.getStatusCode());
        } catch (Exception e) {
            log.error("[EXPO PUSH ERROR] Không thể gửi push notification tới {}: {}", pushToken, e.getMessage());
        }
    }
}
