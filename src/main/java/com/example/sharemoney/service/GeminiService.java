...
public class GeminiService {
    ...
    public AiMessageResponse generateDebtMessage(AiMessageRequest request) {
        if (!isValidApiKey()) {
            log.info("[GeminiService] API key not configured, using smart dynamic heuristic fallback.");
            return AiMessageResponse.builder()
                    .message(generateFallbackMessage(request))
                    .build();
        }
        ...
        try {
            ...
            log.warn("[GeminiService] Gemini API call encountered error: {}. Falling back to dynamic heuristic generator.", e.getMessage());
        }
    ...
    public ScanReceiptResponse extractReceiptFromHtml(String htmlText) {
        if (!isValidApiKey()) {
            log.info("[GeminiService] Gemini API key not set, parsing HTML via heuristic fallback.");
        ...
        try {
            ...
            log.warn("[GeminiService] Gemini HTML extraction failed: {}. Using heuristic parser.", e.getMessage());
        }
    ...
    private ResponseEntity<Map> callGeminiApiWithRetry(HttpEntity<Map<String, Object>> entity) {
        ...
        for (int i = 0; i < maxRetries; i++) {
            try {
                ...
                log.warn("[GeminiService] Attempt {}/{} failed: {}", i + 1, maxRetries, e.getMessage());
                if (i == maxRetries - 1) {
                    throw e;
                }
                try {
    ...
    /**
     * Thuật toán Heuristic đa dạng tạo câu nhắc nợ thông minh cục bộ khi chưa có Gemini API Key hoặc mạng lỗi.
     */
    private String generateFallbackMessage(AiMessageRequest request) {
        ...
        switch (mood) {
            case "POLITE":
                options = List.of(
                        String.format("Chào %s, bạn kiểm tra giúp mình khoản chi tiêu nhóm %s và chuyển khoản giùm mình khi thuận tiện nhé. Cảm ơn bạn rất nhiều! ☕", debtor, amountStr),
                        String.format("Hi %s, khi nào rảnh bạn chuyển khoản %s tiền quỹ nhóm giúp mình nha. Cảm ơn bạn nhiều! 😊", debtor, amountStr),
                        String.format("%s ơi, phiền bạn check lại thông tin chuyển khoản %s tiền nhóm đợt rồi giúp mình nha. Thank you! 🙏", debtor, amountStr),
                        String.format("Gửi %s, gửi bạn thông tin khoản chia hóa đơn %s, bạn sắp xếp chuyển khoản giúp mình nhé. Cảm ơn bạn! ✨", debtor, amountStr)
                );
                break;
            case "AGGRESSIVE":
                options = List.of(
                        String.format("Thông báo khẩn! %s chuyển ngay %s tiền nhóm giúp mình để chốt sổ tài chính nhé! ⚠️⚡", debtor, amountStr),
                        String.format("%s ơi, khoản nợ %s tiền nhóm hơi lâu rồi đó nha, chuyển khoản gấp trong hôm nay giúp mình với! 🛑", debtor, amountStr),
                        String.format("Cảnh báo hạn chót! %s thanh toán ngay %s tiền nhóm nha, không là bị bêu tên trên nhóm đấy! 😡📢", debtor, amountStr),
                        String.format("Alo alo %s! Nhanh tay chuyển khoản %s tiền nợ nhóm giúp mình cái nào, trốn lâu quá rồi đó! 🔥", debtor, amountStr)
                );
                break;
            case "POETIC":
                options = List.of(
                        String.format("Nắng chiều ngả bóng hoàng hôn, tiền nợ %s xin đừng lãng quên hỡi %s 🌸📜", amountStr, debtor),
                        String.format("Trăm năm Kiều vẫn là Kiều, nợ tiền %s nhóm phải liều mà đòi nha %s 🍃💸", amountStr, debtor),
                        String.format("Gió đưa cành trúc la đà, %s chưa trả %s là ta buồn thiu 🎋✨", debtor, amountStr),
                        String.format("Trăng lên đỉnh núi trăng tà, tiền nợ %s mau mau trả nè %s 🌙🕊️", amountStr, debtor)
                );
                break;
            case "FUNNY":
            default:
                options = List.of(
                        String.format("Ê %s, ví tao đang thở oxy nè! Còn %s tiền nhóm hôm bữa, bắn qua cứu bạn hiền với! 🚑💨", debtor, amountStr),
                        String.format("Alo %s ơi, chim sẻ gọi đại bàng! Đại bàng chuyển khoản %s tiền nhóm giùm chim sẻ mua gói mì tôm nhé 😂🍜", debtor, amountStr),
                        String.format("%s iu quý! Bữa ăn/chi tiêu vui quá, giờ đến tiết mục thanh toán %s tiền nhóm kìa! Bắn lẹ nha bồ tèo 💸✨", debtor, amountStr),
                        String.format("Nghe nói người đẹp/soái ca như %s không bao giờ để nợ qua đêm! Chuyển %s tiền nhóm nha người anh em! 🕶️🔥", debtor, amountStr),
                        String.format("Ê %s, mầy còn nợ tao %s tiền nhóm đó nha! Trả lẹ đi mậy, hết tiền ăn sáng rồi! 🥐☕", debtor, amountStr)
                );
                break;
        }
    ...
    private ScanReceiptResponse extractReceiptViaHeuristic(String htmlText) {
        ...
        try {
            // Tìm các mẫu số tiền thường gặp trong hoá đơn tiếng Việt (ví dụ: Tổng tiền: 150.000 hoặc 150,000 đ)
            Pattern pattern = Pattern.compile("(?i)(?:tổng cộng|tổng tiền|thanh toán|total)[^0-9]{1,20}([0-9]{1,3}(?:[.,][0-9]{3})+|[0-9]{4,9})");
            ...
            } else if (htmlText.toLowerCase().contains("co.opmart") || htmlText.toLowerCase().contains("coopmart")) {
                note = "Co.opmart";
            } else if (htmlText.toLowerCase().contains("seven eleven") || htmlText.toLowerCase().contains("7-eleven")) {
                note = "7-Eleven Store";
            }
        } catch (Exception e) {
