...
public class PayOSService {
    ...
    public Map<String, Object> createPaymentLink(
        ...
        try {
            String safeDesc =
                    description != null
                            ? description.replaceAll("[^a-zA-Z0-9 ]", "").trim()
                            : "SM" + orderCode;
            ...
            // Chuỗi dữ liệu ký SHA256 theo thứ tự alphabet: amount, cancelUrl, description,
            // orderCode, returnUrl
            String signData =
                    String.format(
            ...
            if (root.has("code") && "00".equals(root.get("code").asText())) {
                ...
                result.put(
                        "accountNumber",
                        data.has("accountNumber")
                                ? data.get("accountNumber").asText()
                                : "96886693050620");
                result.put(
                        "accountName",
                        data.has("accountName")
                                ? data.get("accountName").asText()
                                : "DUONG DUC BAO");
                result.put("amount", amount);
                result.put(
                        "description",
                        data.has("description") ? data.get("description").asText() : safeDesc);
        ...
        fallback.put(
                "qrCode",
                "https://img.vietqr.io/image/970426-96886693050620-compact2.png?amount="
                        + amount
                        + "&addInfo="
                        + safeDesc);
        return fallback;
    ...
    /**
     * Truy vấn thông tin trạng thái Payment Link trực tiếp từ PayOS Server GET
    ...
    public Map<String, Object> getPaymentLinkInformation(long orderCode) {
        try {
            ...
            HttpResponse<String> response =
                    httpClient.send(request, HttpResponse.BodyHandlers.ofString());
            log.info(
                    "[PayOS Query] OrderCode: {}, Status: {}, Body: {}",
                    orderCode,
                    response.statusCode(),
                    response.body());
            ...
            if (root.has("code") && "00".equals(root.get("code").asText())) {
                ...
                result.put(
                        "amountPaid",
                        data.has("amountPaid") ? data.get("amountPaid").asLong() : 0L);
                return result;
            }
        } catch (Exception e) {
