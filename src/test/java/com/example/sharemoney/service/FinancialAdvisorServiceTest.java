...
class FinancialAdvisorServiceTest {
    ...
    void testGenerateRebalancePlan_NoOverspending() {
        BudgetSummaryResponse food = BudgetSummaryResponse.builder()
        ...
        BudgetSummaryResponse social = BudgetSummaryResponse.builder()
    ...
    @Test
    @DisplayName("Tái cân bằng: Khi có khoản tiêu lố -> Tự động tính toán cắt giảm danh mục linh hoạt để bù vào")
    void testGenerateRebalancePlan_WithOverspending() {
        ...
        // Tiền điện: hạn mức 1.000.000, đã chi 600.000 -> Còn dư 400.000đ (CỐ ĐỊNH -> KHÔNG CẮT GIẢM)
        BudgetSummaryResponse electric = BudgetSummaryResponse.builder()
        ...
        RebalancePlan plan = advisorService.generateRebalancePlan(Arrays.asList(food, social, electric));
        ...
        assertEquals(new BigDecimal("500000"), plan.getOverspentItems().get(0).getOverspentAmount());
        ...
        assertEquals(new BigDecimal("1000000"), cut.getNewSuggestedLimit()); // 1.500.000 - 500.000 = 1.000.000
        assertEquals(new BigDecimal("500000"), plan.getTotalCompensated());
        assertEquals(BigDecimal.ZERO, plan.getRemainingDeficit());
    }

    @Test
    @DisplayName("Tái cân bằng V2: Ưu tiên cắt giảm Tier 1 Luxury (Mua sắm) trước khi đụng vào Tier 2 Basic (Ăn uống)")
    void testGenerateRebalancePlan_TieredPriorityCut() {
        // Tiền nhà (Cố định): hạn mức 2.000.000, đã chi 2.300.000 -> Tiêu lố 300.000đ
        BudgetSummaryResponse house = BudgetSummaryResponse.builder()
        ...
        BudgetSummaryResponse shopping = BudgetSummaryResponse.builder()
        ...
        RebalancePlan plan = advisorService.generateRebalancePlan(Arrays.asList(house, shopping, food));
        ...
        CompensateCutItem shoppingCut = plan.getCompensationCuts().stream()
        ...
        CompensateCutItem foodCut = plan.getCompensationCuts().stream()
    ...
    void testApplyRebalance_WithCustomOverride() {
        com.example.sharemoney.dto.request.RebalanceApplyRequest request =
                com.example.sharemoney.dto.request.RebalanceApplyRequest.builder()
                        .cuts(Collections.singletonList(
                                com.example.sharemoney.dto.request.RebalanceApplyRequest.RebalanceCutOverride.builder()
        ...
        when(budgetService.getBudgetSummary(userId, 2026, 8)).thenReturn(Collections.singletonList(food));
        ...
        when(budgetRepository.findByUser_IdAndCategory_IdAndMonthAndYear(userId, catSocialId, 8, 2026))
